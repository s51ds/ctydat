...
func parseCtyDatRecord(ctyDatRecord string) (ctyDatList []CtyDta, err error) {
	...
	ctyDatList = make([]CtyDta, len(aliasRecords))
	primaryDta := CtyDta{}
	...
	if len(fields) != 9 {
		return []CtyDta{}, errorWrongFormattedRecord("Unexpected number of fields: "+strconv.Itoa(len(fields)), ctyDatRecord)
	}
	...
	if cq, err := strconv.Atoi(strings.TrimSpace(fields[1])); err != nil {
		//TODO: test
		return []CtyDta{}, errorWrongFormattedRecord("Wrong formatted CQ Zone: "+fields[1], ctyDatRecord)
	} else {
	...
	if itu, err := strconv.Atoi(strings.TrimSpace(fields[2])); err != nil {
		//TODO: test
		return []CtyDta{}, errorWrongFormattedRecord("Wrong formatted ITU Zone: "+fields[2], ctyDatRecord)
	} else {
	...
	if c, err := adif.Continent(strings.TrimSpace(fields[3])); err != nil {
		//TODO: test
		return []CtyDta{}, errorWrongFormattedRecord(err.Error(), ctyDatRecord)
	} else {
	...
	if err != nil {
		//TODO: test
		return []CtyDta{}, errorWrongFormattedRecord("Wrong formatted latitude: "+fields[4], ctyDatRecord)
	}
	lon, err := strconv.ParseFloat(strings.TrimSpace(fields[5]), 64)
	if err != nil {
		//TODO: test
		return []CtyDta{}, errorWrongFormattedRecord("Wrong formatted longitude: "+fields[5], ctyDatRecord)
	}
		...
		if pfx != "" && pfx != primaryPfx { //Alias DXCC prefixes always include the primary one
			...
			if overrideCqZone != "" {
				...
				if cqZone != "" {
					if i, e := strconv.Atoi(cqZone); e != nil {
						//TODO: test
						return []CtyDta{}, errorWrongFormattedRecord("wrong formatted override CQ Zone: "+cqZone+" "+e.Error(), ctyDatRecord)
					} else {
						aliasDta.CqZone = adif.CqzoneEnum(i)
					}
				} else {
					//TODO: test
					return []CtyDta{}, errorWrongFormattedRecord("wrong formatted override CQ Zone: "+cqZone, ctyDatRecord)
				}
			}
			//
			...
			if overrideItuZone != "" {
				...
				if ituZone != "" {
					if i, e := strconv.Atoi(ituZone); e != nil {
						//TODO: test
						return []CtyDta{}, errorWrongFormattedRecord("wrong formatted override ITU Zone: "+ituZone+" "+e.Error(), ctyDatRecord)
					} else {
						aliasDta.ItuZone = adif.ItuzoneEnum(i)
					}
				} else {
					//TODO: test
					return []CtyDta{}, errorWrongFormattedRecord("wrong formatted override ITU Zone: "+ituZone, ctyDatRecord)
				}
			}
			//
			...
			if overrideLatLon != "" {
				...
				if errLat != nil || errLon != nil {
					return []CtyDta{}, errorWrongFormattedRecord("wrong formatted override Latitude/Longitude: "+overrideLatLon, ctyDatRecord)
				} else {
...
func parseCtyDatRecords(ctyDatRecords string) (m map[string]CtyDta, err error) {
	m = make(map[string]CtyDta)
	ctyDatRecords = removeComments(ctyDatRecords)
	records := strings.Split(ctyDatRecords, ";")

	numberOfRecords := len(records)
	ch := make(chan []CtyDta, numberOfRecords)
